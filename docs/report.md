...
**Автор:** Тульбович Артём, студент 3 курса факультета искусственного интеллекта.

## Аннотация (RU)

В работе реализован алгоритм градиентного бустинга над решающими деревьями «с
нуля» на NumPy и проведено его сравнение с библиотекой `scikit-learn` при
идентичных гиперпараметрах. На датасетах Diabetes (регрессия) и Breast Cancer
(классификация) собственная реализация отстаёт от эталонной в пределах ~0.1% по
RMSE и совпадает по accuracy, что подтверждает корректность реализации.
Дополнительно исследовано влияние темпа обучения (shrinkage) на переобучение.
Ниже приведён полный текст отчёта на английском языке.
